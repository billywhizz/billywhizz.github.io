...
function open (dbName) {
  ...
  const xhr = new XMLHttpRequest()
  xhr.open("GET", 'quills.wasm', false)
  xhr.responseType = 'arraybuffer'
  xhr.send(null)
  const module = new WebAssembly.Module(xhr.response)
  ...
  const db = {
    ...
    checkpoint: (name = 'main') => {
      ...
      return { rc, logSize: logSize.get(), framesCheckpointed: framesCheckpointed.get() }
    },
    status: () => {
      const results = []
...
onmessage = e => {
  ...
  if (e.data.checkpoint) {
    ...
    const { rc, logSize, framesCheckpointed } = db.checkpoint(name)
    postMessage({ rc, logSize, framesCheckpointed, checkpoint: { wal: db.walInfo(name) } })
    return
  }
  if (e.data.serialize) {
    const db = databases[e.data.db]
